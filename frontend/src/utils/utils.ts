...
export const GetBlogs = ()=>{
    ...
    const { data, error, isLoading } = useSWR(`${API_URL}/api/blogs?populate=*`, fetcher)
    const blogs = data?.data.sort((a: any, b: any) => {
...
export const GetBlog = (id: number) => {
    ...
    const { data, error, isLoading } = useSWR(`${API_URL}/api/blogs/${id}?populate=*`, fetcher)
...
export const Recentblogs = (currentBlogId: number) => {
    ...
    if (blogs && blogs.length > 0) {
        ...
        recentBlogs = blogs
            .filter((blog: any )=> blog.id != currentBlogId) // Assuming each blog has a unique 'id'
            .slice(0, 3);
    }
